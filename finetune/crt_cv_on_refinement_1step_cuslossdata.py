...
def create_datasets(tokenizer, args):
    ...
    def preprocess(example):
        input = f"<commit_start>{example[args.input_column_name].strip()}<commit_msg>Debugging<commit_after>"
        output = f"{example[args.output_column_name].strip()}<eos>"
        input_ids, output_ids = tokenizer([input, output], truncation=False,)["input_ids"]
        all_token_ids = input_ids + output_ids
        all_token_ids_loss_mask = [0] * len(input_ids) + [1] * len(output_ids)
        return {
            "input_ids": all_token_ids,
            "labels": all_token_ids,
            "labels_loss_mask": all_token_ids_loss_mask,
        }
...
def run_training(args, train_data, val_data):
    ...
    training_args = TrainingArguments(
        ...
        run_name="StarCoder-cv-refinement-1step-cuslossdata-fold-{}-seed-{}".format(args.fold, args.seed),
        report_to="wandb",
